...
const APP_BASE = "/fass-dailyscrum/";

/** 개발 서버에서만: 브라우저 → `/fass-dailyscrum/api/jira/*` → JIRA Cloud (Basic 은 서버에서만 주입). */
function buildJiraDevProxy(env: Record<string, string>, mode: string): Record<string, ProxyOptions> | undefined {
  if (mode !== "development") return undefined;
  const baseUrl = env.VITE_JIRA_BASE_URL?.trim();
  ...
  const httpsAgent = tlsInsecure
    ? new https.Agent({ rejectUnauthorized: false })
    : undefined;
  if (tlsInsecure) {
    console.warn(
      "[jira-proxy] TLS verification OFF (JIRA_PROXY_TLS_INSECURE / JIRA_TEST_TLS_INSECURE). Development only."
    );
  }
  return {
    [prefix]: {
      ...
      configure: (proxy) => {
        proxy.on("proxyReq", (proxyReq) => {
          applyJiraProxyRequestHeaders(proxyReq, auth);
        });
        proxy.on("error", (err, _req, res) => {
          const msg = err instanceof Error ? err.message : String(err);
          console.error("[jira-proxy]", msg);
          if (!res || res.headersSent) return;
          const hint =
            /certificate|TLS|SSL|self-signed/i.test(msg) && !tlsInsecure
              ? ".env.local 에 JIRA_PROXY_TLS_INSECURE=1 (또는 JIRA_TEST_TLS_INSECURE=1) 추가 후 개발 서버 재시작"
              : undefined;
...
export default defineConfig(({ mode }) => {
  ...
  const jiraProxy = buildJiraDevProxy(env, mode);
  return {
    base: APP_BASE,
    server: {
      host: true,
