/** 프로젝트 1W 기준 — 2026-05-17 (5월 3주차) */
export const WBS_PROJECT_WEEK_ORIGIN_DATE = new Date(2026, 4, 17, 12, 0, 0, 0);

/** Month 달력 최소 시작 — 2026년 5월 1일 */
...
const ORIGIN_MONDAY = mondayOfWeek(WBS_PROJECT_WEEK_ORIGIN_DATE);

/** Week 달력·0W 라벨이 시작하는 월요일(5월 0W 주) */
export const WBS_TIMELINE_WEEK_START_MONDAY = addWeeks(ORIGIN_MONDAY, -1);

/** gantt-task-react preStepsCount 만큼 앞당긴 뒤 첫 열이 0W(5월)가 되도록 하는 앵커 월요일 */
export function wbsGanttLibraryAnchorMonday(preStepsCount: number): Date {
  return addWeeks(WBS_TIMELINE_WEEK_START_MONDAY, preStepsCount);
}

export function wbsTimelineWeekStartMonday(): Date {
  return new Date(WBS_TIMELINE_WEEK_START_MONDAY.getTime());
}
...
/** Gantt 타임라인 스크롤 상한 — 2027년 12월 4주차(말) */
...
/**
 * WBS·Gantt 달력이 최소한 스크롤되어야 하는 종료일 — 2027년 12월 4주차 일요일.
 */
export function wbsGanttTimelineEndDate(): Date {
  const year = WBS_GANTT_TIMELINE_END_YEAR;
  const monthIndex = WBS_GANTT_TIMELINE_END_MONTH - 1;
...
export function projectWeekIndex(columnWeekMonday: Date): number {
  ...
  const originMon = ORIGIN_MONDAY.getTime();
