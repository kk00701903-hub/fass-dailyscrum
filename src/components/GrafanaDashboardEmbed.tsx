...
export function GrafanaDashboardEmbed() {
  ...
  if (!embedUrl) {
    return (
      <Card className="p-6">
        <SectionHeader
          title="Grafana 시각화"
          subtitle="Share → Embed에서 받은 주소로 대시보드를 iframe에 표시합니다"
          action={
        ...
        <div className="mt-4 rounded-xl border border-orange-200 bg-orange-50 px-4 py-5 text-sm leading-relaxed text-slate-600">
          ...
          <p className="mb-2 text-xs">
            ...
            <em>Share → Embed</em>의 <code className="rounded bg-white px-1 text-[10px]">src</code>와 동일한 전체 URL을
            붙여 넣고 저장하세요.
          </p>
          <p className="mt-3 text-[11px] text-slate-500">
            서버의 <code className="rounded bg-white px-1 text-[10px]">allow_embedding</code> 등으로 iframe이 허용되어야
            합니다.
          </p>
        </div>
      </Card>
  ...
  return (
    <Card className="overflow-hidden p-0">
      <div className={cn(ui.cardHeader, "flex items-center justify-between py-4")}>
        <div>
          <h3 className="text-sm font-semibold tracking-tight text-slate-900">Grafana 대시보드</h3>
          <p className="mt-0.5 break-all pr-2 text-[11px] text-slate-500">{embedUrl}</p>
        </div>
        <a
          href={embedUrl}
          target="_blank"
